...
def advise_cannibalization(cluster_name: str, keywords: list[str], urls: list[dict]) -> Optional[dict]:
    ...
    Returns dict with keys is_cannibalization, verdict_summary, winner_url, winner_reasoning, per_url.
    Returns None if LLM disabled or call failed.
    """
    if not is_enabled() or not urls:
        return None

    url_list = "\n".join(
        f"  - {u.get('slug') or u.get('url')} | type: {u.get('type', 'unknown')} | intent: {u.get('intent_primary') or 'unknown'}"
        for u in urls[:25]  # cap context size
    )
    ...
    raw = _call(_CANNIB_SYSTEM, prompt, max_tokens=2000)
